...
@Component({
    ...
    <div class="page-header">
      <div class="page-title">
        ...
        <div>
          ...
          <p>{{ esEdicion() ? 'Actualiza los datos del usuario' : 'Completa el formulario para registrar un nuevo usuario' }}</p>
        </div>
      </div>
      <a routerLink="/usuarios">
        <p-button label="Volver" icon="pi pi-arrow-left" [text]="true" severity="secondary" />
      </a>
    </div>
        ...
        <div class="form-section">
          ...
          <div class="form-grid">
            ...
            <div class="field">
              ...
              <input pInputText formControlName="nombres" placeholder="Ingresa los nombres" class="w-full" />
              @if (isInvalid('nombres')) {
                <small class="error-msg"><i class="pi pi-exclamation-circle"></i> {{ getError('nombres') }}</small>
              }
            </div>

            <div class="field">
              <label>Apellidos <span class="required">*</span></label>
              <input pInputText formControlName="apellidos" placeholder="Ingresa los apellidos" class="w-full" />
              @if (isInvalid('apellidos')) {
                <small class="error-msg"><i class="pi pi-exclamation-circle"></i> {{ getError('apellidos') }}</small>
              }
            </div>

            <div class="field">
              <label>Cédula</label>
              <input pInputText formControlName="cedula" placeholder="1234567890" class="w-full" maxlength="10" />
            </div>

            <div class="field">
              <label>Teléfono</label>
              <input pInputText formControlName="telefono" placeholder="0991234567" class="w-full" />
            </div>
        ...
        <div class="form-section">
          ...
          <div class="form-grid">
            ...
            <div class="field">
              ...
              @if (isInvalid('correo')) {
                <small class="error-msg"><i class="pi pi-exclamation-circle"></i> {{ getError('correo') }}</small>
              }
            </div>
            ...
            <div class="field">
              <label>
                ...
                @if (esEdicion()) { <small class="hint">(dejar en blanco para no cambiar)</small> }
              </label>
              <p-password
              ...
              @if (isInvalid('contrasena')) {
                <small class="error-msg"><i class="pi pi-exclamation-circle"></i> {{ getError('contrasena') }}</small>
              }
            </div>
            ...
            <div class="field">
              ...
              />
              @if (isInvalid('rolId')) {
                <small class="error-msg"><i class="pi pi-exclamation-circle"></i> El rol es obligatorio</small>
              }
            </div>
  ...
  styles: [`
    ...
    .field label {
      font-size: 0.85rem; font-weight: 600; color: #0a2342;
    }
...
export class UsuarioFormComponent implements OnInit {
  ...
  cargarUsuario(id: number): void {
    this.usuarioService.obtener(id).subscribe({
      ...
      error: () => {
        this.toast.add({ severity: 'error', summary: 'Error', detail: 'No se pudo cargar el usuario' });
  ...
  onSubmit(): void {
    ...
    if (this.esEdicion()) {
      ...
      this.usuarioService.actualizar(this.usuarioId()!, req).subscribe({
        next: () => {
          this.toast.add({ severity: 'success', summary: 'Actualizado', detail: 'Usuario actualizado exitosamente' });
          setTimeout(() => this.router.navigate(['/usuarios']), 1000);
          this.guardando.set(false);
        },
        error: (err) => {
          this.toast.add({ severity: 'error', summary: 'Error', detail: err.error?.mensaje ?? 'No se pudo actualizar' });
          this.guardando.set(false);
        }
      });
    } else {
      this.usuarioService.crear(value).subscribe({
        next: () => {
          this.toast.add({ severity: 'success', summary: 'Creado', detail: 'Usuario creado exitosamente' });
          setTimeout(() => this.router.navigate(['/usuarios']), 1000);
          this.guardando.set(false);
        },
        error: (err) => {
          this.toast.add({ severity: 'error', summary: 'Error', detail: err.error?.mensaje ?? 'No se pudo crear el usuario' });
          this.guardando.set(false);
        }
      });
    }
